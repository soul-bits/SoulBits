...
export function Output({ feedback, fileName, tokensUsed }: OutputProps) {
  ...
  const handleDownloadMarkdown = () => {
    ...
    a.href = url;
    a.download = `resume-feedback-${fileName ? fileName.replace(/\.[^/.]+$/, '') : 'review'}.md`;
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      doc.text('Resume Feedback', margin, yPosition);
      yPosition += 10;
      ...
      // Save PDF
      doc.save(`resume-feedback-${fileName ? fileName.replace(/\.[^/.]+$/, '') : 'review'}.pdf`);
      setShowDownloadMenu(false);
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border rounded-lg p-6 bg-background max-h-[600px] overflow-y-auto prose prose-sm max-w-none">
        <ReactMarkdown className="whitespace-pre-wrap">{feedback}</ReactMarkdown>
      </div>
    </div>
  );
}